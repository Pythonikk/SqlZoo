...
-- how the player, teamid, stadium and mdate for every German goal.
...
SELECT teamname, COUNT(matchid) AS goals_scored
FROM eteam
JOIN goal ON goal.teamid = eteam.id
GROUP BY teamname;

-- Show the stadium and the number of goals scored in each stadium.
SELECT stadium, COUNT(matchid) AS goals_scored
FROM game
JOIN goal ON game.id = goal.matchid
GROUP BY stadium;
...
GROUP BY matchid, mdate, matchid;
